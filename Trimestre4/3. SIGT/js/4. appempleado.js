...
function cambiarEstado(elemento, estadoTexto, claseBoton, iconoUrl) {
  const dropdown = elemento.closest('.dropdown');
  const boton = dropdown.querySelector('button');

  // Quitar clases previas de estado
  boton.classList.remove('btn-success', 'btn-warning', 'btn-danger');

  // Agregar nueva clase
  boton.classList.add(claseBoton);

  // Cambiar contenido
  boton.innerHTML = `<img src="${iconoUrl}" class="me-1"/> ${estadoTexto}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Gráfico de barras (desempeño)
  const ctxDesempeno = document.getElementById('chartDesempeno').getContext('2d');
  new Chart(ctxDesempeno, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Desempeño (%)',
        data: [0, 89],
        backgroundColor: ['#5D87C5FF', '#5D87C5FF']
      }]
    },
    options: {
      responsive: true,
